...
public:
	...
	TestType operator+(TestType other) const {
...
class TestTypeWithoutPlus final {
public:
	TestTypeWithoutPlus() = default;
	TestTypeWithoutPlus(const int& num) : data(num) {}

	TestTypeWithoutPlus operator+(TestTypeWithoutPlus other) const = delete;

	bool operator==(TestTypeWithoutPlus other) const {
		return data == other.data;
	}
private:
	int data;
...
TEST(Class, Type) {
	...
	TestType expectRes(3);
	EXPECT_EQ(expectRes, sum_all(v));
}
